...
void Mesh::Load(const string &fileName) {
	...
	while(!in.eof()) {
		...
		if(token == "joints") {
			...
			while(true) {
				if(in.eof()) FATAL("Error while loading mesh from ",fileName);
				in >> token;
				if(token=="}") break;
			...
			while(true) {
				...
				if(token=="vert") {
					if(numVerts==~0) FATAL("Error while loading mesh from ",fileName);
					...
					if(numTris==~0) FATAL("Error while loading mesh from ",fileName);
					int id; in >> id;
					Tri &tri=tris[id];
					in >> tri.idx[0] >> tri.idx[2] >> tri.idx[1];
				}
				else if(token=="weight") {
					if(numWeights==~0) FATAL("Error while loading mesh from ",fileName);
					int id; string p1,p2; in >> id;
...
void MeshAnim::Load(const string &fileName) {
	...
	while(!in.eof()) {
		...
		if(token=="hierarchy") {
			...
			while(true) {
				if(in.eof()) FATAL("Error while loading mesh anim from ",fileName);
				in >> token;
				if(token=="}") break;
