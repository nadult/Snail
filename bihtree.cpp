...
BIHTree::BIHTree(const vector<Object> &obj) :objects(obj) {
...
uint BIHTree::FindSimilarParent(uint nNode,uint axis) const {
...
void BIHTree::PrintInfo(uint nNode,uint level) const {
	if(nNode==0) {
		for(uint n=0;n<nodes.size();n++) {
		...
		printf("sizeof(tri): %d \n",sizeof(Triangle));
		printf(	"Objects:%8d    Nodes:%8d    Mem:%6.2fMB\n"
				,objects.size(),nodes.size(),double(nodes.size()*sizeof(BIHNode))*0.000001);
...
void BIHTree::Build(uint nNode,int first,int last,Vec3p min,Vec3p max,uint level) {
