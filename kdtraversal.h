...
INLINE void ComputePV(Group &group,const Selector &sel,floatq *pv)
{
...
#define INSERT(id) { int tid=id; \
		const Vec3q dir=VAbs(group.Dir(tid)); \
		Vec3q idir=VAbs(group.IDir(tid)); \
	\
		floatq t1,t2; \
...
template <class Group,class Selector>
void ComputeBeam(Group &group,const Selector &sel,const Vec3p &minOR,const Vec3p &maxOR,
...
void ComputeBeam(Group &group,const Selector &sel,const Vec3p &minOR,const Vec3p &maxOR,
	...
	beamDir=Vec3p(0,0,0);
	Vec3q maxVec=Condition(sel.Mask(0),group.Dir(sel[0]));

	Vec3q tmp=maxVec;
	for(int n=1;n<sel.Num();n++)
		tmp+=Condition(sel.Mask(n),group.Dir(sel[n]));
...
public:
	...
	TraverseContext(Group &group,const Selector &selector,const KDTree &tree,OtherContext *other) {
		sel=selector;
...
inline void KDTree::TraverseFast(Group &group,const Selector &tSelector,const Output &out,OtherContext *tContext) const
{
	if(tSelector.Num()<3) {
	...
	TraverseContext<Group,Selector> c(group,tSelector,*this,tContext);
	...
	while(true) {
		...
		if(axis==3) { // Leaf
			if(c.node->NumObjects()) {
				if(Selector::size>4&&density>1.0f&&c.sel.Num()>(Selector::size/3)&&noSplit==0) { // Too many triangles / ray, splitting
					stats.Skip();
		...
		//			else stats.Breaking();
	...
	
					{
						...
						density=Max(density,r*obj.InvSize()*c.beamM);
					}	
...
void KDTree::TraverseSplitted(Group &group,const Selector &sel,const Output &out,
	...
	if(Output::type==otPrimary) {
		RaySelector<(Selector::size>4?Selector::size/4:Selector::size)> tsel[4];

		for(int n=0;n<sel.Num();n++) {
			int seg=(sel[n]&(Selector::size-1))/(Selector::size/4);
			tsel[seg].Add(sel,n);
		}

		for(int s=0;s<4;s++)
			TraverseFast(group,tsel[s],out,tContext);
		return;
	}
...
template <class Output,class Group,class Selector>
void KDTree::TraverseOptimized(Group &group,const Selector &sel,const Output &out) const {
	if(!sel.Num()) return;
	...
	for(int k=0;k<8;k++) {
		...
		if(sel.Num()) {
			...
			TraverseFast(group,sel,out,(TraverseContext<Group,Selector>*)0);
		}
	}
	if(selectors[8].Num()) {
