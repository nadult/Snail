	...
	void BIHTree::TraverseQuad16Primary(const BIHTravContext &c,int dirMask) const {
		...
		while(true) {
				...
				if(!mailbox.Find(idx)) {
					...
					if(gVals[0]) { //backface culling
						...
						if(ForAll(minDot<=0.0f)) {
							minDot= Max(
								Max(Max(c.dir[ 1]|tnrm,
										Max(c.dir[ 2]|tnrm,c.dir[ 3]|tnrm)),
								Max(Max(c.dir[ 4]|tnrm,c.dir[ 5]|tnrm),
									Max(c.dir[ 6]|tnrm,c.dir[ 7]|tnrm))),
								Max(Max(Max(c.dir[ 8]|tnrm,c.dir[ 9]|tnrm),
										Max(c.dir[10]|tnrm,c.dir[11]|tnrm)),
									Max(Max(c.dir[12]|tnrm,c.dir[13]|tnrm),
			...
			if(node->density*(tMin+tMax)*0.5f>maxDensity) {
				stats.Skip(16);
				if(fStack==fStackBegin) { allTMin=tMin; allTMax=tMax; allIdx=idx; }
